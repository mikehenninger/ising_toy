...
fn main() {
    ...
    // for a in lattice.sites.data.iter() {
    //     println!("{}", a.magnetic_moment);
    // }
    let new_vec_external_field = (-2..2)
        .map(|x| (x as f64) * 0.0)
        .collect::<Vec<f64>>()
    ...
    for idx_t in 0..max_iter {
        ...
        println!("iter: {}", idx_t);
        {
            let actual_sites = lattice.sites.read().unwrap();
            for a in actual_sites.data.iter() {
                println!("{}", a.magnetic_moment);
            }
        }
